...
#else
Conplug_PMS5003T::Conplug_PMS5003T(HardwareSerial *pPmsSerial) {
  PmsSerial = pPmsSerial;
  // PmsSerial->begin(9600);
...
PMS5003T_DATA *Conplug_PMS5003T::readPms() {
  ...
  delay(
      DelayValue[Conplug_PMS5003T::AFTER_SEND_REQUEST_CMD]); // Wait for PMS5003
                                                             // to calculate
                                                             // data.
  ...
  while ((millis() - pms_timeout) < 1300) {
    if (PmsSerial->available()) {
      ...
      delay(DelayValue[Conplug_PMS5003T::SERIAL_READ]); // PMS5003T is a slow
                                                        // sensor
    }
  }
  ...
  while ((millis() - pms_timeout) < 1500) {
    ...
    if (bi >= MAX_PMS_DATA_SIZE)
      break; // exceed buffer size
    if ((data_len != 0) && (bi >= (data_len + 4)))
      break; // exceed data length
...
//
int Conplug_PMS5003T::pm1_0() { return Pd->PM_AE_UG_1_0; }

//
// Running readPms before running pm2_5
//
int Conplug_PMS5003T::pm2_5() { return Pd->PM_AE_UG_2_5; }

//
// Running readPms before running pm10_0
//
int Conplug_PMS5003T::pm10_0() { return Pd->PM_AE_UG_10_0; }

int Conplug_PMS5003T::pm3_0(void) { return Pd->PM_NUM_0_3; }

//
// Running readPms before running temp
//
float Conplug_PMS5003T::temp() { return ((float)(Pd->PM_TEMP)) / 10; }

//
// Running readPms before running humi
//
float Conplug_PMS5003T::humi() { return ((float)(Pd->PM_HUMI)) / 10; }
...
//
void Conplug_PMS5003T::setDelay(Conplug_PMS5003T::PMS_DELAY PmsDelay,
                                int Value) {
