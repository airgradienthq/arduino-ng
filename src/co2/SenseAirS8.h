 ...
 * @brief Arigradient libarary for Senseair S8 NDIR CO2 sensor, this libaray port base
 * on library from https://github.com/jcomas/S8_UART
 * @version 0.1
 * @date 2023-Dec-16
 *
...
{
public:
  const int S8_BAUDRATE    = 9600;    // Device to S8 Serial baudrate (should not be changed)
  const int S8_TIMEOUT     = 5000ul;  // Timeout for communication in milliseconds
  const int S8_LEN_BUF_MSG = 20;      // Max length of buffer for communication with the sensor
  ...
  enum Status
  {
    ...
    S8_MASK_METER_ANY_ERROR               = 0x007F,   // Mask to detect the previous errors (fatal error ... memory error)
  };
  enum OutputStatus
  {
...
private:
  ...
  uint8_t uartReadBytes(uint8_t max_bytes, uint32_t timeout_seconds); // Read received bytes from sensor
  bool validResponse(uint8_t func, uint8_t nb);                          // Check if response is valid according to sent command
  bool validResponseLenght(uint8_t func, uint8_t nb, uint8_t len);         // Check if response is valid according to sent command and checking expected total length
